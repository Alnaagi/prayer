...
// late DateTime fajrStartTime = DateTime.now();
// late DateTime fajrEndTime = DateTime.now();
...
// late DateTime dhuhrStartTime = DateTime.now();
// late DateTime dhuhrEndTime = DateTime.now();

DateTime? dhuhrStartTime;
DateTime? dhuhrEndTime;

/// The time for the Asr (afternoon) prayer's start and end.
// late DateTime asrStartTime = DateTime.now();
// late DateTime asrEndTime = DateTime.now();

DateTime? asrStartTime;
DateTime? asrEndTime;

/// The time for the Maghrib (evening) prayer's start and end.
// late DateTime maghribStartTime = DateTime.now();
// late DateTime maghribEndTime = DateTime.now();
DateTime? maghribStartTime;
DateTime? maghribEndTime;

/// The time for the Isha (night) prayer's start and end.
// late DateTime ishaStartTime = DateTime.now();
// late DateTime ishaEndTime = DateTime.now();
...
PrayerTimes({bool precision = false, DateTime? dateTime}) {
  ...
  fajrTime = PrayerTimeConverter(
      ...
      .utcDate(date.year, date.month, date.day);
  fajrafterTime = PrayerTimeConverter(
          solarTimeAfter.hourAngle(-1 * calculationParameters.fajrAngle, false))
      .utcDate(dateAfter.year, dateAfter.month, dateAfter.day);
  ...
  if (calculationParameters.ishaInterval != null &&
    ...
    DateTime safeIsha() {
      if (calculationParameters.method == "MoonsightingCommittee") {
        return CelestialMath.seasonAdjustedEveningTwilight(coordinates.latitude,
            CelestialTimeUtils.dayOfYear(date), date.year, sunsetTime);
      } else {
  ...
  if (calculationParameters.maghribAngle != null) {
    DateTime angleBasedMaghrib = PrayerTimeConverter(
